...
- Name: UI/Settlements/setl-panel-tc1-bank-details-panel-visible.spec.ts >> Settlements — Selected Bank Details Panel >> TC1 — "Selected Bank Details" panel is visible
- Location: tests/UI/Settlements/setl-panel-tc1-bank-details-panel-visible.spec.ts:20:7
...
Locator: getByText(/Selected Bank Details/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/Selected Bank Details/i)
...
- main:
  ...
  - text: 9 Settled 20 Payout Completed
...
```ts
  ...
  27 | 
  28 |     await logger.step('Step 1 — Verifies the Selected Bank Details side panel renders on the Settlements page', async () => {
  29 |       logger.info('Test scope: Verifies the Selected Bank Details side panel renders on the Settlements page');
  30 |     });
  31 | 
  ...
  46 |     });
  47 | 
  48 |     await logger.step('Step 4 — "Selected Bank Details" panel heading is visible', async () => {
  49 |       logger.info('Checking Selected Bank Details panel is rendered');
> 50 |       await expect(settlementsPage.selectedBankDetailsPanel).toBeVisible();
     |                                                              ^ Error: expect(locator).toBeVisible() failed
  51 |       logger.pass('Selected Bank Details panel is visible and accessible');
  52 |       await attachScreenshot(page, testInfo, '03 — Selected Bank Details panel visible', settlementsPage.selectedBankDetailsPanel);
  53 |     });
  54 | 
  55 |     // NOTE: placeholder copy updated to match the current UI — the empty-state
  56 |     // text is "select the settlement data", not "Select a row to see details"
  57 |     // (confirmed live, 2026-07).
  58 |     await logger.step('Step 5 — Panel shows "select the settlement data" placeholder', async () => {
  59 |       logger.info('Asserting visibility: select the settlement data placeholder text');
  60 |       await expect(page.getByText(/select the settlement data/i)).toBeVisible();
  61 |       logger.pass('select the settlement data placeholder text is visible');
  62 |       await attachScreenshot(page, testInfo, '04 — Select a row placeholder visible', page.getByText(/select the settlement data/i));
  63 |     });
  64 |   });
  65 | });
  66 | 
