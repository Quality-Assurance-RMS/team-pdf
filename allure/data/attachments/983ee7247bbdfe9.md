...
```ts
  ...
  29 | 
  30 |   test('TC5 — All 12 month labels (Jan–Dec) are present in the chart', async ({ page, logger }, testInfo: TestInfo) => {
  31 |     // ── Allure metadata ──────────────────────────────────────────────────────
  ...
  41 | 
  42 |     await logger.step('Step 1 — Navigate to Dashboard page', async () => {
  43 |       logger.info('Opening the Dashboard page');
  44 |       await dashboard.goto();
  45 |       logger.pass('Navigation to Dashboard initiated successfully');
  46 |       await attachScreenshot(page, testInfo, '01 — Page loading');
  47 |     });
  48 | 
  49 |     await logger.step('Step 2 — Verify page loads successfully', async () => {
  50 |       logger.info('Waiting for all Dashboard elements to render');
  51 |       await dashboard.assertPageLoaded();
  52 |       logger.pass('Dashboard page is fully loaded with all required elements');
  53 |       await attachScreenshot(page, testInfo, '02 — Page loaded and ready');
  54 |     });
  55 | 
  56 |     // ── Full list of expected X-axis month labels ────────────────────────────
  57 |     const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  58 | 
  59 |     // ── Assertions: one step per month for clear failure identification ───────
  60 |     let stepNum = 3;
  61 |     for (const month of months) {
  62 |       const stepLabel = `Step ${stepNum}`;
  63 |       const screenshotPrefix = String(stepNum).padStart(2, '0');
  64 |       await logger.step(`${stepLabel} — Month label "${month}" is visible`, async () => {
  65 |         // exact: true prevents "Jan" matching "January" if the UI uses full names
  66 |         logger.info(`Asserting visibility: ${month} month label on X-axis`);
> 67 |         await expect(page.getByText(month, { exact: true })).toBeVisible({ timeout: 5000 });
     |                                                              ^ Error: expect(locator).toBeVisible() failed
  68 |         logger.pass(`${month} month label is visible`);
  69 |         await attachScreenshot(page, testInfo, `${screenshotPrefix} — ${month} label visible`, page.getByText(month, { exact: true }));
  70 |       });
  71 |       stepNum++;
  72 |     }
  73 |   });
