...
- Name: UI/Transactions/status-filter-dropdown.spec.ts >> Transactions — Status Filter Dropdown >> TC2 — Status filter dropdown is visible
- Location: tests/UI/Transactions/status-filter-dropdown.spec.ts:19:7
...
Locator: locator('select').filter({ hasText: /All Status/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('select').filter({ hasText: /All Status/i }).first()
...
```ts
  1  | /**
  2  |  * @file status-filter-dropdown.spec.ts
  3  |  * @testId TXN-FLT-TC2
  4  |  * @feature Transactions — Status Filter
  5  |  * @priority P1 / Critical
  6  |  *
  7  |  * Verifies that the Status filter dropdown is visible on the Transactions page.
  8  |  */
  9  | 
  ...
  15 | 
  16 | test.describe('Transactions — Status Filter Dropdown', () => {
  17 |   test.setTimeout(120000);
  18 | 
  19 |   test('TC2 — Status filter dropdown is visible', async ({ page, logger }, testInfo: TestInfo) => {
  20 |     await allure.description('Verifies the "All Status" filter dropdown is rendered on the Transactions page.');
  21 |     await allure.label('feature',  'Transactions');
  22 |     await allure.label('story',    'Status Filter');
  23 |     await allure.label('severity', 'critical');
  24 |     await allure.label('priority', 'P1');
  25 |     await allure.label('testId',   'TXN-FLT-TC2');
  26 | 
  ...
  41 |     });
  42 | 
  43 |     await logger.step('Step 3 — Status filter showing "All Status" is visible', async () => {
  ...
  46 |       await expect(
  47 |         page.locator('select').filter({ hasText: /All Status/i }).first()
> 48 |       ).toBeVisible();
     |         ^ Error: expect(locator).toBeVisible() failed
  49 |       logger.pass('Filter dropdown is visible and accessible');
  50 |       await attachScreenshot(page, testInfo, '03 — Status filter All Status visible', page.locator('select').filter({ hasText: /All Status/i }).first());
  51 |     });
  52 |   });
  53 | });
  54 | 
