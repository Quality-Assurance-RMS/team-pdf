...
- Name: API/smoke/smoke-dashboard-revenue-trend.spec.ts >> Smoke â€” Dashboard Revenue Trend >> TC-DREVT-012 â€” BUG-REVT-11: CORS wildcard origin is insecure for financial trend data
- Location: tests/API/smoke/smoke-dashboard-revenue-trend.spec.ts:955:7

# Error details

```
Error: [BUG-REVT-11a] Access-Control-Allow-Origin must not be wildcard "*" â€” must list specific trusted origins
...
```
Error: [BUG-REVT-11b] CORS must not allow DELETE on GET-only endpoint
...
```
Error: [BUG-REVT-11b] CORS must not allow POST on GET-only endpoint
