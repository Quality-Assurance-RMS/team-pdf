...
- Name: API/backend-tests/backend-transaction-list.spec.ts >> Backend (Konza) â€" Transaction List >> TC-BE-005 â€" List: period=month filtered by status=SUCCESS and method=MPESA
- Location: tests/API/backend-tests/backend-transaction-list.spec.ts:237:7

# Error details

```
Error: TC-BE-005 â€" HTTP 200 expected
