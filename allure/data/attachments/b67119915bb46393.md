...
```ts
  ...
  19 | 
  20 |   test('TC1 — "Selected Bank Details" panel is visible', async ({ page }, testInfo: TestInfo) => {
  ...
  38 |     });
  39 | 
  40 |     await test.step('Step 3 — "Selected Bank Details" panel heading is visible', async () => {
  41 |       await expect(settlementsPage.selectedBankDetailsPanel).toBeVisible();
  42 |       await attachScreenshot(page, testInfo, '03 — Selected Bank Details panel visible', settlementsPage.selectedBankDetailsPanel);
  43 |     });
  44 | 
  45 |     await test.step('Step 4 — Panel shows "Select a row to see details" placeholder', async () => {
> 46 |       await expect(page.getByText(/Select a row to see details/i)).toBeVisible();
     |                                                                    ^ Error: expect(locator).toBeVisible() failed
  47 |       await attachScreenshot(page, testInfo, '04 — Select a row placeholder visible', page.getByText(/Select a row to see details/i));
  48 |     });
  49 |   });
  50 | });
  51 | 
