# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: UI/Settlements/heading-visible.spec.ts >> Settlements — Page Heading >> TC1 — "Settlements" heading and count are visible
- Location: tests/UI/Settlements/heading-visible.spec.ts:20:7

# Error details

```
Error: expect(locator).toBeVisible() failed

Locator: locator('h1, h2, h3').filter({ hasText: /^Settlements$/i }).or(getByText('Settlements').first())
Expected: visible
Error: strict mode violation: locator('h1, h2, h3').filter({ hasText: /^Settlements$/i }).or(getByText('Settlements').first()) resolved to 2 elements:
    1) <span class="text-md font-medium">Settlements</span> aka getByRole('link', { name: 'Settlements' })
    2) <h2 class="text-xl sm:text-2xl text-gray-600 font-semibold">Settlements</h2> aka getByRole('heading', { name: 'Settlements' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('h1, h2, h3').filter({ hasText: /^Settlements$/i }).or(getByText('Settlements').first())

```

# Page snapshot

```yaml
- generic [ref=e2]:
  - generic [ref=e3]:
    - complementary [ref=e4]:
      - img "eCitizen" [ref=e8] [cursor=pointer]
      - navigation [ref=e9]:
        - link "Dashboard" [ref=e10] [cursor=pointer]:
          - /url: /dashboard
          - img [ref=e11]
          - generic [ref=e13]: Dashboard
        - link "Transactions" [ref=e14] [cursor=pointer]:
          - /url: /transaction
          - img [ref=e15]
          - generic [ref=e17]: Transactions
        - link "Settlements" [ref=e18] [cursor=pointer]:
          - /url: /settlements
          - img [ref=e19]
          - generic [ref=e21]: Settlements
        - link "Reports" [ref=e22] [cursor=pointer]:
          - /url: /reports
          - img [ref=e23]
          - generic [ref=e25]: Reports
        - link "User Management" [ref=e26] [cursor=pointer]:
          - /url: /user
          - img [ref=e27]
          - generic [ref=e29]: User Management
        - link "Settings" [ref=e30] [cursor=pointer]:
          - /url: /settings
          - img [ref=e31]
          - generic [ref=e33]: Settings
      - button "Logout" [ref=e35] [cursor=pointer]:
        - img [ref=e36]
        - generic [ref=e38]: Logout
      - button "Collapse sidebar" [ref=e39]:
        - img [ref=e40]
    - generic [ref=e42]:
      - banner [ref=e43]:
        - generic [ref=e44]:
          - button "Notifications" [ref=e45]:
            - img [ref=e46]
            - generic [ref=e49]: "3"
          - generic [ref=e52]: A
          - generic [ref=e54]:
            - generic [ref=e55]: Ashil
            - generic [ref=e56]:
              - generic [ref=e57]: SUPER_ADMIN
              - generic [ref=e58]: PLATFORM
      - main [ref=e60]:
        - generic [ref=e61]:
          - generic [ref=e62]:
            - generic [ref=e64]:
              - heading "Settlements" [level=2] [ref=e65]
              - paragraph [ref=e66]: 0 settlements have been initiated.
            - combobox [ref=e68]:
              - option "Today"
              - option "Last 7 Days" [selected]
              - option "Last 30 Days"
              - option "Last 90 Days"
              - option "Custom Range"
          - generic [ref=e69]:
            - generic [ref=e70]:
              - generic [ref=e71]:
                - img "icon" [ref=e73]
                - generic [ref=e74]:
                  - heading "Gross Collected" [level=2] [ref=e75]
                  - text: 0% vs prior 7 days
              - generic [ref=e77]:
                - generic [ref=e78]:
                  - img [ref=e80]
                  - generic [ref=e88]:
                    - generic [ref=e89]: KES 0
                    - generic: KES 0.00
                - generic [ref=e90]:
                  - img [ref=e92]
                  - generic [ref=e102]:
                    - generic [ref=e103]: USD 0
                    - generic: USD 0.00
            - generic [ref=e104]:
              - generic [ref=e105]:
                - img "icon" [ref=e107]
                - generic [ref=e108]:
                  - heading "Total Fees" [level=2] [ref=e109]
                  - paragraph [ref=e110]: Avg 0%
              - generic [ref=e112]:
                - generic [ref=e113]:
                  - img [ref=e115]
                  - generic [ref=e123]:
                    - generic [ref=e124]: KES 0
                    - generic: KES 0.00
                - generic [ref=e125]:
                  - img [ref=e127]
                  - generic [ref=e137]:
                    - generic [ref=e138]: USD 0
                    - generic: USD 0.00
            - generic [ref=e139]:
              - generic [ref=e140]:
                - img "icon" [ref=e142]
                - generic [ref=e143]:
                  - heading "Net Settlement" [level=2] [ref=e144]
                  - paragraph [ref=e145]: Settlement Activity Summary
              - generic [ref=e147]:
                - generic [ref=e148]:
                  - img [ref=e150]
                  - generic [ref=e158]:
                    - generic [ref=e159]: KES 0
                    - generic: KES 0.00
                - generic [ref=e160]:
                  - img [ref=e162]
                  - generic [ref=e172]:
                    - generic [ref=e173]: USD 0
                    - generic: USD 0.00
            - generic [ref=e174]:
              - generic [ref=e175]:
                - img "icon" [ref=e177]
                - heading "Last Settlement" [level=2] [ref=e179]
              - generic [ref=e185]:
                - text: No settlement available
                - text: for the selected period
          - generic [ref=e186]:
            - paragraph [ref=e187]: Settlement by Bank
            - generic [ref=e188]:
              - table [ref=e191]:
                - rowgroup [ref=e192]:
                  - row "Service Name Gross Collected Fees Net Settlement Last Settlement Status" [ref=e193]:
                    - columnheader "Service Name" [ref=e194]
                    - columnheader "Gross Collected" [ref=e195]
                    - columnheader "Fees" [ref=e196]
                    - columnheader "Net Settlement" [ref=e197]
                    - columnheader "Last Settlement" [ref=e198]
                    - columnheader "Status" [ref=e199]
                - rowgroup [ref=e200]:
                  - row "No Data Found" [ref=e201]:
                    - cell "No Data Found" [ref=e202]
              - generic [ref=e203]:
                - heading "Selected Bank Details" [level=3] [ref=e204]
                - paragraph [ref=e205]: select the settlement data
  - region "Notifications alt+T"
```

# Test source

```ts
  1  | /**
  2  |  * @file heading-visible.spec.ts
  3  |  * @testId SETL-HDR-TC1
  4  |  * @feature Settlements — Page Heading
  5  |  * @priority P0 / Blocker
  6  |  *
  7  |  * Verifies that the "Settlements" heading and the initiated-count sub-heading
  8  |  * are visible after navigating to /settlements.
  9  |  */
  10 | 
  11 | import { test, expect } from '../fixtures';
  12 | import type { TestInfo } from '@playwright/test';
  13 | import { allure } from 'allure-playwright';
  14 | import { SettlementsPage } from '../../pages/SettlementsPage';
  15 | import { attachScreenshot } from '../../utils/screenshotHelper';
  16 | 
  17 | test.describe('Settlements — Page Heading', () => {
  18 |   test.setTimeout(120000);
  19 | 
  20 |   test('TC1 — "Settlements" heading and count are visible', async ({ page }, testInfo: TestInfo) => {
  21 |     await allure.description('Verifies the Settlements heading and "X settlements have been initiated" text render on page load.');
  22 |     await allure.label('feature',  'Settlements');
  23 |     await allure.label('story',    'Page Heading');
  24 |     await allure.label('severity', 'blocker');
  25 |     await allure.label('priority', 'P0');
  26 |     await allure.label('testId',   'SETL-HDR-TC1');
  27 | 
  28 |     const settlementsPage = new SettlementsPage(page);
  29 | 
  30 |     await test.step('Step 1 — Navigate to Settlements page', async () => {
  31 |       await settlementsPage.goto();
  32 |       await attachScreenshot(page, testInfo, '01 — Page loading');
  33 |     });
  34 | 
  35 |     await test.step('Step 2 — Verify page loads successfully', async () => {
  36 |       await settlementsPage.assertPageLoaded();
  37 |       await attachScreenshot(page, testInfo, '02 — Page loaded and ready');
  38 |     });
  39 | 
  40 |     await test.step('Step 3 — "Settlements" heading is visible', async () => {
> 41 |       await expect(settlementsPage.heading).toBeVisible();
     |                                             ^ Error: expect(locator).toBeVisible() failed
  42 |       await attachScreenshot(page, testInfo, '03 — Settlement heading visible', settlementsPage.heading);
  43 |     });
  44 | 
  45 |     await test.step('Step 4 — Settlement initiated count is visible', async () => {
  46 |       await expect(settlementsPage.settlementSubCount).toBeVisible();
  47 |       await attachScreenshot(page, testInfo, '04 — Settlement initiated count visible', settlementsPage.settlementSubCount);
  48 |     });
  49 |   });
  50 | });
  51 | 
```