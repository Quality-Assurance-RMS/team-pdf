...
```ts
  ...
  28 | 
  29 |   test('TC3 — Date filter dropdown defaults to "Today"', async ({ page }, testInfo: TestInfo) => {
  30 |     // ── Allure metadata ──────────────────────────────────────────────────────
  ...
  50 | 
  51 |     // ── Assertion ────────────────────────────────────────────────────────────
  52 |     await test.step('Step 3 — "Today" label is visible in the date filter', async () => {
  53 |       // exact: true prevents matching "Today's Totals" or similar substrings
> 54 |       await expect(page.getByText('Today', { exact: true })).toBeVisible({ timeout: 10000 });
     |                                                              ^ Error: expect(locator).toBeVisible() failed
  55 | 
  56 |       // Record confirmed default value in the Allure report
  57 |       allure.parameter('Default filter value', 'Today');
  58 |       await attachScreenshot(page, testInfo, '03 — Date filter defaults to Today', page.getByText('Today', { exact: true }));
  59 |     });
  60 |   });
  61 | });
  62 | 
