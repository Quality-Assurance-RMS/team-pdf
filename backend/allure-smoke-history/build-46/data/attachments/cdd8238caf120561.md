...
- Name: API/smoke/smoke-dashboard-today-payments.spec.ts >> Smoke â€" Dashboard Today Payments >> TC-DTPAY-005 â€" BUG-TPAY-03/04: isPositive flag must agree with postiveValue percentage
- Location: tests/API/smoke/smoke-dashboard-today-payments.spec.ts:537:7
...
> 552 |       expect(res!.status).toBe(200);
  ...
  626 | 
  627 |       // payments: isPositive=false and postiveValue="-100.0%" â€" this IS consistent
  628 |       const isConsistent =
  629 |         (flag === true  && typeof pct === 'string' && !pct.startsWith('-')) ||
  630 |         (flag === false && typeof pct === 'string' && pct.startsWith('-'));
  631 | 
  632 |       allure.parameter('payments isPositive/postiveValue consistent', String(isConsistent));
  633 |       logger.info('Asserting: payments.isPositive is consistent with postiveValue sign (informational)');
  634 |       // This assertion is informational â€" payments section currently looks consistent
  635 |       logger.pass('All assertions passed');
  636 |     });
  637 |   });
  638 | 
  639 |   // â"€â"€ TC-DTPAY-006 â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  640 |   test('TC-DTPAY-006 â€" BUG-TPAY-05: Wrong HTTP methods must return 405, not 500', async ({ logger }) => {
  641 |     smokeLabels('TC-DTPAY-006', 'BUG-TPAY-05 â€" HTTP Method Enforcement', 'normal');
  642 | 
  643 |     const wrongMethods = ['post', 'put', 'delete', 'patch'] as const;
  644 | 
  645 |     for (const method of wrongMethods) {
  646 |       await logger.step(`Step â€" ${method.toUpperCase()} must return 405, not 500`, async () => {
  647 |         logger.info(`${method.toUpperCase()} /api/v1/dashboard/today-payments`);
  648 |         const res = await getWithMethod(method);
  649 |         logger.pass('HTTP ' + res.status + ' received');
  650 | 
  651 |         console.log(`[TC-DTPAY-006] ${method.toUpperCase()}  -> ${res.status}`);
  652 |         console.log(`  Body: ${JSON.stringify(res.body)}`);
```