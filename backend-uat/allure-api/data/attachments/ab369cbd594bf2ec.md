...
- Name: API/backend-tests/backend-analytics-dashboard-summary.spec.ts >> Backend (Konza) – Analytics Dashboard Summary >> TC-ASUM-002 – BUG-ASUM-01 (BLOCKER): Complete auth bypass – endpoint is publicly accessible
- Location: tests/API/backend-tests/backend-analytics-dashboard-summary.spec.ts:228:7

# Error details

```
Error: [BUG-ASUM-01] "No Authorization header" must return 401 Unauthorized, got 502
...
```
Error: [BUG-ASUM-01] "Invalid Bearer token" must return 401 Unauthorized, got 502
...
```
Error: [BUG-ASUM-01] "Bearer empty string" must return 401 Unauthorized, got 502
...
```
Error: [BUG-ASUM-01] "Bearer null literal" must return 401 Unauthorized, got 502
...
```
Error: [BUG-ASUM-01] "Wrong scheme (Basic)" must return 401 Unauthorized, got 502
...
```
Error: [BUG-ASUM-01] "Clearly expired token" must return 401 Unauthorized, got 502
