...
- Name: API/oauth-token-spr-fetcher-api.spec.ts >> API Gateway & Security >> [BUG-GW-002] TC-GW-003 — OPTIONS preflight must not allow arbitrary origins (e.g. https://evil.com)
- Location: tests/API/oauth-token-spr-fetcher-api.spec.ts:1452:7

# Error details

```
Error: [BUG-GW-002] Preflight must not allow arbitrary origins, got: "*"
