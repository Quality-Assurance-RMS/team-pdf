...
- Name: API/smoke/smoke-dashboard-revenue-trend.spec.ts >> Smoke â€” Dashboard Revenue Trend >> TC-DREVT-015 â€” Response-Time SLA: must respond within 2 000 ms
- Location: tests/API/smoke/smoke-dashboard-revenue-trend.spec.ts:1218:7
...
> 1240 |       expect(res!.status).toBe(200);
  ...
  1249 |         );
  1250 |       }
  1251 |       logger.info('Asserting: response structure and values');
  1252 |       expect(durationMs, `Response time ${durationMs}ms must be under ${SLA_MS}ms`).toBeLessThan(SLA_MS);
  1253 |       logger.pass('All assertions passed');
  1254 |     });
  1255 |   });
  1256 | 
  1257 | });
