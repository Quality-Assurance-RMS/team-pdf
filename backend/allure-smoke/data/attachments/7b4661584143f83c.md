...
- Name: API/smoke/smoke-dashboard-summary.spec.ts >> Smoke â€” Dashboard Summary >> TC-DSUM-015 â€” BUG-SUM-12/13/14: latency SLA, cache headers, CORS configuration
- Location: tests/API/smoke/smoke-dashboard-summary.spec.ts:934:7
...
> 950  |         expect(res.status, `Run ${i + 1} must return 200`).toBe(200);
  ...
  1024 |         );
  1025 |       }
  1026 |       expect.soft(advertised.length,
  1027 |         `[BUG-SUM-14] Allow-Methods must not list destructive methods â€” currently advertises: [${advertised.join(', ')}]`,
  1028 |       ).toBe(0);
  1029 | 
  1030 |       allure.parameter('CORS Allow-Origin',  allowOrigin    ?? 'MISSING');
  1031 |       allure.parameter('CORS Allow-Methods', allowMethods   || 'MISSING');
  1032 |       allure.parameter('Dangerous methods advertised', advertised.join(', ') || 'none âœ“');
  1033 |       logger.pass('All assertions passed');
  1034 |     });
  1035 |   });
  1036 | 
  1037 | });
