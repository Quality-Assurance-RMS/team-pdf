...
```ts
  ...
  19 | 
  20 |   test('TC1 — "Settled" status badge is visible in the table', async ({ page }, testInfo: TestInfo) => {
  ...
  38 |     });
  39 | 
  40 |     await test.step('Step 3 — At least one "Settled" badge is visible', async () => {
> 41 |       await expect(settlementsPage.settledBadge).toBeVisible({ timeout: 15000 });
     |                                                  ^ Error: expect(locator).toBeVisible() failed
  42 |       await attachScreenshot(page, testInfo, '03 — Settled badge visible', settlementsPage.settledBadge);
  43 |     });
  44 |   });
  45 | });
  46 | 
