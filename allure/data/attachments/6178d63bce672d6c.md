...
> 70 |     await expect(this.page).toHaveURL(/\/settlements/, { timeout: 15000 });
  ...
  81 | 
  82 |   /**
  83 |    * Reads the top row of the Settlement batch table (Batch Name, Settlement
  84 |    * Date, Start Time, End Time, Currency, Net Amount, Status columns) — newly
  85 |    * created batches sort to the top, so this is this run's batch.
  86 |    */
  87 |   async getTopBatch(): Promise<{ name: string; currency: string; status: string }> {
  88 |     const topRow = this.tableRows.first();
  89 |     await expect(topRow).toBeVisible({ timeout: 15000 });
  90 |     const cells = topRow.locator('td');
  91 |     return {
  92 |       name:     (await cells.nth(0).innerText()).trim(),
  93 |       currency: (await cells.nth(4).innerText()).trim(),
  94 |       status:   (await cells.nth(6).innerText()).trim(),
  95 |     };
  96 |   }
  97 | }
  98 | 
