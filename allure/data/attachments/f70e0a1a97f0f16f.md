...
```ts
  ...
  28 | 
  29 |   test('TC4 — Clicking "Refunds" navigates to the Refunds page', async ({ page }, testInfo: TestInfo) => {
  30 |     // ── Allure metadata ──────────────────────────────────────────────────────
  ...
  40 | 
  41 |     await test.step('Step 1 — Navigate to Dashboard page', async () => {
  42 |       await dashboard.goto();
  43 |       await attachScreenshot(page, testInfo, '01 — Page loading');
  44 |     });
  45 | 
  46 |     await test.step('Step 2 — Verify page loads successfully', async () => {
  47 |       await dashboard.assertPageLoaded();
  48 |       await attachScreenshot(page, testInfo, '02 — Page loaded and ready');
  49 |     });
  50 | 
  51 |     // ── Step 3: Click the nav item ───────────────────────────────────────────
  52 |     await test.step('Step 3 — Click Refunds nav item', async () => {
> 53 |       await dashboard.navRefunds.first().click();
     |                                          ^ Error: locator.click: Test timeout of 120000ms exceeded.
  54 |       await attachScreenshot(page, testInfo, '03 — Refunds nav item clicked');
  55 |     });
  56 | 
  57 |     // ── Step 4: Verify the URL ───────────────────────────────────────────────
  58 |     await test.step('Step 4 — URL contains /refund', async () => {
  59 |       await expect(page).toHaveURL(/\/refund/, { timeout: 10000 });
  60 |       allure.parameter('URL', page.url());
  61 |       await attachScreenshot(page, testInfo, '04 — Navigated to Refunds page');
  62 |     });
  63 |   });
  64 | });
  65 | 
