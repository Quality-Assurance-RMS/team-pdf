...
- Name: API/transactions/pg-webhook-transactions.spec.ts >> RMS — PG Webhook Transactions (supertest) >> TC4 — Expired requestExpiryTimestamp — Validation
- Location: tests/API/transactions/pg-webhook-transactions.spec.ts:574:7

# Error details

```
Error: Expected HTTP 400 Bad Request
...
```ts
  ...
  527 |     await allure.label('story',    'Security — HMAC Validation');
  528 |     await allure.label('severity', 'blocker');
  529 |     await allure.label('priority', 'P0');
  530 |     await allure.label('testId',   'TC3');
  531 |     await allure.label('type',     'Security / Negative');
  532 | 
  533 |     const ZEROED_SIG = '0'.repeat(64);
  534 |     const payload    = buildInvalidSignaturePayload();
  535 | 
  536 |     // ── Step 1: Send webhook with zeroed-out signature ───────────────────
  537 |     await logger.step('Step 1 — POST /transactions with zeroed-out X-Response-Signature', async () => {
  538 |       logger.info('POST /pgwebhook/transactions (zeroed-out signature)');
  539 |       const res = await postWebhook(payload, ZEROED_SIG);
  540 |       logger.pass('HTTP ' + res.status + ' received');
  541 | 
  542 |       allure.parameter('X-Response-Signature', ZEROED_SIG);
  543 |       allure.parameter('HTTP status',          String(res.status));
  544 | 
  545 |       logger.info('Asserting: HTTP 401 Unauthorized');
  546 |       expect(res.status, 'Expected HTTP 401 Unauthorized').toBe(401);
  547 |       logger.pass('All assertions passed');
  548 |     });
  549 | 
  550 |     // ── Step 2: Verify nothing stored in PostgreSQL ──────────────────────
  551 |     await logger.step('Step 2 — Verify PostgreSQL: 0 rows for TXN-INVALID001', async () => {
  552 |       const rows = await queryPgTransactions({ payment_reference: 'TXN-INVALID001' });
  553 | 
  554 |       allure.parameter('Row count', String(rows.length));
  555 |       logger.info('Asserting: 0 rows stored after 401');
  556 |       expect(rows.length, '0 rows — nothing stored after 401').toBe(0);
  557 |       logger.pass('All assertions passed');
  558 |     });
  559 | 
  560 |     // ── Step 3: Verify nothing stored in MongoDB ─────────────────────────
  561 |     await logger.step('Step 3 — Verify MongoDB: 0 documents in raw_pg_events for this idempotency_key', async () => {
  562 |       const docs = await queryMongoEvents({ idempotency_key: 'aaaabbbb-cccc-dddd-eeee-ffffaaaabbbb' });
  563 | 
  564 |       allure.parameter('MongoDB document count', String(docs.length));
  565 |       logger.info('Asserting: 0 documents in MongoDB after rejection');
  566 |       expect(docs.length, '0 documents — rejected before any write').toBe(0);
  567 |       logger.pass('All assertions passed');
  568 |     });
  569 |   });
  570 | 
  571 |   // ──────────────────────────────────────────────────────────────────────────
  572 |   // TC4 — Expired requestExpiryTimestamp — Validation
  573 |   // ──────────────────────────────────────────────────────────────────────────
  574 |   test('TC4 — Expired requestExpiryTimestamp — Validation', async ({ logger }) => {
  575 |     await allure.description(
  576 |       'Verifies that a webhook whose requestExpiryTimestamp is in the past is ' +
  577 |       'rejected with HTTP 400 bad_request. ' +
  578 |       'The HMAC signature is valid; only the timestamp check fails. ' +
